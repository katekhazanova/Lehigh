...
<#--
   Added by jon, 6/1/07
   returns true if the current binding result has any errors registered, false otherwise
   -->
<#function hasErrors>
    <#if status.errorMessages?size gt 0>
    	<#return true />
    </#if>
	<#return false />
</#function>
...
<#-- modified on 6/10/07 by jon to check if fieldType == "file", and if so, not display the value (you don't want to display the binary) -->
    <@bind path/>
    <input type="${fieldType}" id="${status.expression}" name="${status.expression}"  class="form-control" <#if fieldType != "file">value="<#if fieldType!="password">${stringStatusValue?html}</#if>"</#if> ${attributes}
    <@closeTag/>
</#macro>
...
<#macro formTextarea path attributes="" >
    <@bind path/>
    <textarea class="form-control" id="${status.expression}" name="${status.expression}" ${attributes}>${stringStatusValue?html}</textarea>
</#macro>
...
<#macro formSingleSelect path options attributes="">
    <@bind path/>
    <select class="form-control " id="${status.expression}" name="${status.expression}" ${attributes}>
...
<#macro formMultiSelect path options attributes="">
    <@bind path/>
    <select multiple class="form-control" id="${status.expression}" name="${status.expression}" ${attributes}>
        <#list options?keys as value>
...
-->
    ...
    <#-- edited by jon, 5/31/07, to insert hidden field -->
    <input type="hidden" value="on" name="_${status.expression}" <@closeTag/>
...
<#--
 * added by Jon 3/26/2007
 *
 * formCheckbox
 *
 * Show one checkbox.  Also prints out a hidden form field to indicate if the status of a checkbox.
 *
 * @param path the name of the field to bind to
 * @param option a boolean value representing if the box should be checked
 * @param attributes any additional attributes for the element (such as class
 *    or CSS styles or size
-->
<#macro formCheckbox path option attributes="">
    <@bind path/>
    <#assign id=status.expression />
    <input type="checkbox" id="${id}" name="${id}" value="${option}" 
    	<#if status.value> checked="checked" </#if>
    /><input type="hidden" name="_${id}" />
    
    <#--
    	#macro( formCheckboxInput $path $option $label )
		    #springBind($path)
		    <input type="checkbox" id="${status.expression}" name="${status.expression}" value="${option}"
		        #if( $status.value ) checked="checked" #end
			/><input type="hidden" name="_${status.expression}" /> $label <br />#springShowErrors( '<br />' 'error' )
		#end		
    -->
...
<#macro showErrors separator classOrStyle="">
    ...
    <#else>
        ...
        <span ${attr}="${classOrStyle}" role="alert" tabindex="0">${error}</span>
    </#if>
...
<#function contains list item>
    ...
    <#-- 6/1/07 jon, added ".id" to only compare on id's, therefore this will only work on objects that have an id field 
    <#if nextInList == item><#return true></#if>
    -->
    <#if nextInList.id?? && nextInList.id == item><#return true></#if>
    </#list>
    <#return false>
</#function>
