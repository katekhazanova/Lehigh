...
<@layout.body hideMenu=true>
	...
	<div class="col-md-3 col-lg-3 hidden-xs hidden-sm">
		...
		<h2>Options</h2>
			<ul id="admin-sidebar" class="nav nav-pills nav-stacked">
				...
				<li><a href="<@s.url relativeUrl="/general/helloworld.html#user" />">Current User Details</a></li>
				<li><a href="<@s.url relativeUrl="/general/helloworld.html#appURLs" />">Application URLs</a></li>
				<li><a href="<@s.url relativeUrl="/general/helloworld.html#version" />">Version Information</a></li>
			...
			<tbody>
				...
				<#list AppFunRoleValues.values() as ss>
					<#assign v = ss?string>
						...
						<th>${v}</th>
						<td>
							<@security.authorize ifAnyGranted="${v}=c" >
						...
						<td>
							<@security.authorize ifAnyGranted="${v}=r" >
								<i class="fa fa-check" style="color: green;font-size: 16px;" title="True"><span class="sr-only">You have this application function</span></i>
							</@security.authorize>
						...
						<td>
							<@security.authorize ifAnyGranted="${v}=u" >
								<i class="fa fa-check" style="color: green;font-size: 16px;" title="True"><span class="sr-only">You have this application function</span></i>
							</@security.authorize>
					 ...
					 <td>
							<@security.authorize ifAnyGranted="${v}=d" >
								<i class="fa fa-check" style="color: green;font-size: 16px;" title="True"><span class="sr-only">You have this application function</span></i>
							</@security.authorize>
	...
	<hr />
	<#if ! isStudent??>
	<h2 id="user">Current User Details</h2>
	<#if defaultInst?? >
	<h3>Configuration</h3>
	<div class="section">
			<table  id="currentUserTable" class="table table-striped table-bordered">
				<thead>
					<tr>
					<th> Institutions </th>
					<th> COM Key Configuration Strategy: ${comKeyStrategy} </th>
					</tr>
			...
			<tbody>
				<tr>
					...
					<#if institutionsConfigured??>
					<#list institutionsConfigured as institution>
							<tr>
								<td>
									${institution.instcd.code}
								</td>
								<td>
									 ${institution.instcd.name} 
								</td>
							</tr>
					</#list>
					<#else>
		...
		<h3>User defined</h3>
			...
			<table  id="currentUserTable" class="table table-striped table-bordered">
				<thead>
					...
					<th> Username </th>
					<th> SOPRID </th>
					<th> Logged in Institution </th>
					<th> Auth Code : Description </th>
					<th> COM Key</th>
					</tr>
			...
			<tbody>
					...
					<td>
					<#if authCodeList??>
					<#list authCodeList as authCodes>
						${authCodes.value!''} : ${authCodes.description!'No Description'}</br>
					</#list>
					</#if>
					</td>
					<td>
...
<#else>
	<div class="section">
		...
		<h2 id="appURLs">Application URLs</h2>
		<div class="section">
			<table  id="appURLS" class="table table-striped table-bordered">
						<thead>
							<#if dashboardURL??>
					<tr>
					<th> Dashboard URL </th>
