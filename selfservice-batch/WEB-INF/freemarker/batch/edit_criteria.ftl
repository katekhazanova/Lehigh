<@layout.head title="Edit Criteria" />
...
<@layout.body>
	...
	<form id="createCriteria" class="focusable" action="<@h.url href="/batch/edit_criteria.html" />" method="post">		<table class="verticalListing">
					...
					<tr>
						<th>Operand</th>
						<td class="content">
						<#if !operandOps[editOperand?string]?has_content>
							<span id="operand_spinner" class="error">
								"${editOperand}" is invalid!
							</span>
						</#if>
									<select class="form-control" name="operand" id="operand" >
											<#list operandOps?keys as key>  
  												<option  value="${key}" <#if editOperand?? && key == editOperand>selected</#if>>
  													${operandOps[key]}
  												</option>
											</#list>
									</select>
					...
					<tr>
						<th>Value</th>
						<td class="content">
									<input aria-labelledby="chooseFromListLabel" type="radio" id="valueRadio" name="valueRadio" value="dropDown" <#if editValueRadio?? && editValueRadio == "dropDown">checked</#if>/>
									<span id="chooseFromListLabel">Choose from list</span>
									<select class="form-control" name="value" id="value"  onchange="updateRadioOnClick('value')">
											<#list valueOps?keys as key>  
  												<option  value="${key}" <#if editValue?? && key == editValue>selected</#if>>
  													${valueOps[key]}
  												</option>
											</#list>
									</select>
									...
									<br/>
									<br/>
									<input aria-labelledby="manuallyEnterLabel" type="radio" id="valueTextRadio" name="valueRadio" value="text" <#if editValueRadio?? && editValueRadio == "text">checked</#if>/>
									<span id="manuallyEnterLabel">Manually enter</span>
									<input type="text" class="form-control" name="valueText" id="valueText" <#if editValueText??>value="${editValueText}"</#if> onchange="updateRadioOnClick('valueText')"/>
						</td>
						<td>
								&nbsp;
						</td>
