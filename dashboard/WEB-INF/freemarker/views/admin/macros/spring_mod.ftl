...
<#-- modified on 6/10/07 by jon to check if fieldType == "file", and if so, not display the value (you don't want to display the binary) -->
    <@bind path/>
    <input type="${fieldType}" id="${status.expression}" name="${status.expression}" <#if fieldType != "file">value="<#if fieldType!="password">${stringStatusValue}</#if>"</#if> ${attributes}
    <@closeTag/>
</#macro>
...
<#macro formCheckbox path option attributes="">
    ...
    <input type="checkbox" id="${id}" name="${id}" value="${option}" ${attributes}
