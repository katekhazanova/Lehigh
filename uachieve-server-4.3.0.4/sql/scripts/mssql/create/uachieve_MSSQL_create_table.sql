...
/*                                                                */
/* Script name:    uachieve_MSSQL_create_tab.sql                  */
/* Purpose:        Creates the u.achieve database tables          */
/*                                                                */
...
create table job_queue_accept (
   int_seq_no           numeric(18)           identity(1, 1),
   jobq_seq_no          numeric(18)           not null default 0,
   ...
   user_seq_no          numeric(18)           not null  default 0,
   dept                 char(15)              null,
   crsno                char(15)              null,
...
create table job_queue_detail (
   ...
   jobid                char(16)              not null,
   userid               char(30)              not null,
   user_seq_no          int                   not null default 0,
   stuno                char(13)              null,
   comkey               char(5)               null,
   evalsw               char(1)               null,
   f1or2                char(1)               null,
...
create table job_queue_exceptions (
   ...
   last_mod_user        varchar(30)           null,
   last_mod_date        datetime              null,
   constraint PK_J_Q_EXCEPTION primary key (jobq_seq_no, user_seq_no)
)
go
...
create table job_queue_markers (
   ...
   constraint PK_J_Q_MARKER primary key (jobq_seq_no, user_seq_no)
)
go
...
create table job_queue_report (
   ...
   report               image               null,
   constraint PK_J_Q_REPORT primary key (jobq_seq_no, report_type)
)
go
...
create table string (
   ...
   stypeq               char(1)               null,
   cond                 char(1)               null,
   limct                numeric(3)            null default 0,
...
/* ==== View: vw_stu_demo                                            */
create view vw_stu_demo with Encryption as
...
/* ==== View: vw_stu_master                                          */
create view vw_stu_master with Encryption as
