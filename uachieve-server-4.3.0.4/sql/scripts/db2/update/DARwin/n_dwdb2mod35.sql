...
create index stu_info_idx on stu_master (
   stuno                ASC,
   instidq              ASC,
   instid               ASC
);
...
alter table job_queue_course
   add constraint FK_Reference_901 foreign key (jobq_seq_no, rtabx, stabx)
      references job_queue_subreq (jobq_seq_no, rtabx, stabx)
      on delete restrict on update restrict;
...
alter table job_queue_out
   add constraint FK_Reference_902 foreign key (jobq_seq_no)
      references job_queue_run (int_seq_no)
      on delete restrict on update restrict;

alter table job_queue_req
   add constraint FK_Reference_903 foreign key (jobq_seq_no)
      references job_queue_run (int_seq_no)
      on delete restrict on update restrict;

alter table job_queue_subreq
   add constraint FK_Reference_904 foreign key (jobq_seq_no, rtabx)
